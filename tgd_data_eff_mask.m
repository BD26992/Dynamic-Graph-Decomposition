function out = tgd_data_eff_mask(X,A_true,Mask,W,prms)
...
for r=1:R
        ...
        Ab = my_proj_symmetric_verify(Ab)./(eta+alpha*J);
        A(:,r)=vec(Ab);
end
%A = double(A > 0);
...
mxt = mt.*xt;
Bt = mt.*kron(A,eye(T));
F = alpha*(Bt'*Bt)+mu*kron(eye(R),Dt'*Dt) + rho*kron(eye(R),eye(T));
c = F\(alpha*Bt'*mxt-delta*vec(S));%C update
%%%%%%%%%%%
